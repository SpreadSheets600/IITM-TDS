...
async def fetch(ctx, url):
    ...
    try:
        ...
        f.write_text(html)
        print("GET", url, len(html), flush=True)
        return html
    finally:
        await page.close()
...
def normalize_product_url(sku):
    return f"{BASE}/49/p/{sku}.html"


def get_sku_from_card(card):
    t = text(card)
    m = re.search(r"SM-[A-Z]+-\d+", t)
    ...
    while todo:
        ...
        for a in s.select("a[href]"):
            ...
            if kind == "ecom":
                ...
                ok = path.startswith("/69/c/tech")
            elif kind == "news-business":
                ok = path.startswith("/69/c/business")
            elif kind == "news-author":
                ok = path.startswith("/69/author/author_21")
            elif kind == "social":
                ok = path.startswith("/82/") and not any(
                    x in path for x in ["/u/", "/user/", "/post/"]
                )
            elif kind == "forum-users":
                ok = path.startswith("/6/users")
            elif kind == "opsec":
                ok = path.startswith("/6/b/opsec")
            if ok and v not in seen and v not in todo:
                todo.append(v)
...
async def product_details(ctx, list_pages, category_filter=None):
    ...
    out = {}
    for sku in skus:
        h = await fetch(ctx, normalize_product_url(sku))
        s = soup(h)
        d = s.select_one(".p-detail") or s
        t = text(d)
        cat = text(d.select_one(".p-info > div")).lower()
        ...
        if old is None:
            vals = [
                Decimal(x.replace(",", ""))
                for x in re.findall(r"\$\s*(\d[\d,]*(?:\.\d+)?)", t)
            ]
        ...
        if sd:
            ...
            if m:
                stock = int(m.group(1))
        r = None
        rs = d.select_one(".rating-strip[aria-label]")
        if rs:
...
def article_items(pages):
    ...
    for u, s in pages:
        for el in s.select("[data-internal-views]"):
            item = el
            for p in el.parents:
                if getattr(p, "name", None) in ("article", "div", "li", "tr") and (
                    p.get("data-article-id") or p.get("id") or p.select_one("a[href]")
                ):
                    item = p
                    break
...
def parse_social_users(pages):
    ...
    for u, s in pages:
        for card in s.select(
            "[data-location], [data-followers], .user-card, .profile-card, .card, article, tr"
        ):
            t = text(card)
            if not (card.get("data-followers") or re.search(r"followers?", t, re.I)):
                continue
            h = (
                card.get("data-handle")
                or card.get("data-username")
                or card.get("data-user")
            )
            if not h:
                m = re.search(r"@([\w.-]+)", t)
            ...
            fol = card.get("data-followers") or (
                re.search(r"followers?\D*(\d[\d,]*)", t, re.I).group(1)
                if re.search(r"followers?\D*(\d[\d,]*)", t, re.I)
                else "0"
            )
            out[h.lstrip("@")] = {
                "text": t,
    ...
    for u, s in pages:
        for card in s.select("[data-likes], .post, article, .card"):
            ...
            if "#ai" not in t.lower():
                continue
            likes = card.get("data-likes") or (
                re.search(r"likes?\D*(\d[\d,]*)", t, re.I).group(1)
                if re.search(r"likes?\D*(\d[\d,]*)", t, re.I)
                else "0"
            )
            pid = (
...
def parse_forum_users(pages):
    ...
    for u, s in pages:
        for card in s.select(
            "[data-reputation], [data-rep], .user-card, .member-card, tr, article, .card"
        ):
            t = text(card)
            if not (
                card.get("data-reputation")
                or card.get("data-rep")
                or re.search(r"\b(rep|reputation)\b", t, re.I)
            ):
                continue
...
def parse_threads(pages):
    ...
    for u, s in pages:
        for card in s.select("[data-views], .thread, .thread-row, tr, article, .card"):
            t = text(card)
            if not (card.get("data-views") or re.search(r"views?", t, re.I)):
                continue
            v = card.get("data-views") or (
                re.search(r"views?\D*(\d[\d,]*)", t, re.I).group(1)
                if re.search(r"views?\D*(\d[\d,]*)", t, re.I)
                else None
            )
            if not v:
                continue
            ...
            if not tid and a:
                tid = a["href"]
            m = re.search(r"(?:thread[_-]?|/t/|thread/)([A-Za-z0-9_-]+)", tid)
            shown = m.group(1) if m else tid
            out[shown or t[:50]] = {
                "id": shown,
                "views": int(str(v).replace(",", "")),
                "text": t,
            }
    return list(out.values())
...
async def main():
    async with async_playwright() as p:
        ...
        home_pages = await crawl_list_pages(
            ctx, BASE + "/49/cat/home/index.html", "ecom"
        )
        apparel_pages = await crawl_list_pages(
            ctx, BASE + "/49/cat/apparel/index.html", "ecom"
        )
        store_pages = await crawl_list_pages(ctx, BASE + "/49/index.html", "ecom")
        tech_pages = await crawl_list_pages(
            ctx, BASE + "/69/c/tech/index.html", "news-tech"
        )
        business_pages = await crawl_list_pages(
            ctx, BASE + "/69/c/business/index.html", "news-business"
        )
        tiffany_pages = await crawl_list_pages(
            ctx, BASE + "/69/author/author_21.html", "news-author"
        )
        ...
        forum_user_pages = await crawl_list_pages(
            ctx, BASE + "/6/users/index.html", "forum-users"
        )
        opsec_pages = await crawl_list_pages(
            ctx, BASE + "/6/b/opsec/index.html", "opsec"
        )
        ...
        tech = article_items(tech_pages)
        bus = article_items(business_pages)
        tif = article_items(tiffany_pages)
        social = parse_social_users(social_pages)
        ai = parse_ai_likes(explore_pages)
        fusers = parse_forum_users(forum_user_pages)
        threads = parse_threads(opsec_pages)
        oos = [
            p["rating"]
        ...
        res = {
            "task1": str(
                sum(
                    (p["current"] or Decimal(0)) * Decimal(p["stock"])
                    for p in home.values()
            ...
            "task3": str(
                (sum(oos, Decimal(0)) / Decimal(len(oos))).quantize(
